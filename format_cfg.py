...
def format_file(
    ...
    if new_contents != contents:
        new_filename = filename.replace('setup.cfg', 'setup_formatted.cfg')
...
def format_single_file(filename, include_version_classifiers=False, min_py_version=(3, 8), max_py_version=(3, 12)):
    if format_file(
            filename,
            include_version_classifiers=include_version_classifiers,
...
def format_cfg(filename):
    print('\n---------------- format_cfg.py ----------------')


    result = format_single_file(filename, include_version_classifiers=True, min_py_version=(3, 6), max_py_version=(3, 13))
    # print(f'Result: {result}')

if __name__ == '__main__':
    format_cfg(r'C:\.PythonProjects\SavedTests\_test_projects_for_building_packages\_extra_files_to_copy\setup.cfg')